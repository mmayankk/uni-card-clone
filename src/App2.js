import logo from './logo.svg';
import './App.css';

function App() {
  return (
<>
  <meta httpEquiv="Content-Type" content="text/html; charset=UTF-8" />
  <link
    rel="preload"
    href="https://www.uni.cards/fonts/Matter-Medium.woff2"
    as="font"
    type="font/woff2"
    crossOrigin=""
  />
  <meta
    name="viewport"
    content="width=device-width, initial-scale=1, viewport-fit=cover"
  />
  <meta name="theme-color" content="#000000" />
  <title>Uni Cards: Next-gen Credit Cards &amp; Rewards</title>
  <link rel="icon" href="https://www.uni.cards/images/favicon.ico" />
  <meta
    name="description"
    content="Elevate your credit experience with the next-gen 🌊NX Wave Credit Card | Uni Paychek: receive a salary before your salary | Uni PowerUp: Earn consistent 9% returns on your money"
  />
  <meta
    property="og:title"
    content="Uni Cards: Next-gen Credit Cards & Rewards"
  />
  <meta
    property="og:description"
    content="Elevate your credit experience with the next-gen 🌊NX Wave Credit Card | Uni Paychek: receive a salary before your salary | Uni PowerUp: Earn consistent 9% returns on your money"
  />
  <meta property="og:image" content="/images/card_preview.jpeg" />
  <meta name="og:type" content="website" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:site" content="@cards_uni" />
  <meta
    name="twitter:title"
    content="Uni Cards: Next-gen Credit Cards & Rewards"
  />
  <meta
    name="twitter:description"
    content="Elevate your credit experience with the next-gen 🌊NX Wave Credit Card | Uni Paychek: receive a salary before your salary | Uni PowerUp: Earn consistent 9% returns on your money"
  />
  <meta name="twitter:image" content="/images/card_preview.jpeg" />
  <meta name="next-head-count" content={15} />
  <noscript data-n-css="" />
  
  <meta
    httpEquiv="origin-trial"
    content=""
  />
  <meta
    httpEquiv="origin-trial"
    content=""
  />
  <meta
    httpEquiv="origin-trial"
    content=""
  />
  <noscript>
    &lt;iframe src="https://www.googletagmanager.com/ns.html?id=GTM-M8J7HKL"
    height="0" width="0"
    style="display:none;visibility:hidden"&gt;&lt;/iframe&gt;
  </noscript>
  <div id="__next" data-reactroot="">
    <header >
      <div style={{ background: "transparent" }}>
        <div >
          <div
            style={{ cursor: "pointer" }}
            data-testid="logo"
            href="/"
            
          >
            <svg
              
              viewBox="0 0 85 60"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M54.894 39.5039H48.6641V19.77H54.894V39.5039ZM30.4021 25.9999V39.504H24.1723V22.3647C24.1723 21.6765 24.4457 21.0165 24.9322 20.5301C25.4188 20.0435 26.0787 19.77 26.7669 19.77H42.3312C43.0195 19.77 43.6793 20.0435 44.1657 20.5301C44.6523 21.0165 44.9257 21.6765 44.9257 22.3647V39.504H38.696V25.9999H30.4021ZM14.2345 33.2742V19.7702H20.4644V36.9095C20.4644 37.5977 20.1909 38.2576 19.7044 38.744C19.2179 39.2307 18.5579 39.5041 17.87 39.5041H2.30544C1.61752 39.5041 0.957379 39.2307 0.470894 38.744C-0.0157201 38.2576 -0.289062 37.5977 -0.289062 36.9095V19.7702H5.9407V33.2742H14.2345Z"
                fill="black"
              />
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M65.289 19.7708C65.289 21.8685 63.5858 23.5717 61.4882 23.5717C59.3905 23.5717 57.6875 21.8685 57.6875 19.7708C57.6875 17.6732 59.3905 15.9702 61.4882 15.9702C63.5858 15.9702 65.289 17.6732 65.289 19.7708Z"
                fill="black"
              />
            </svg>
          </div>
          <div >
            <div >
              <div >
                <a
                  href="https://paychek.uni.club/"
                  
                >
                  Uni Paychek
                </a>
                <div  />
              </div>
            </div>
            <div >
              <button
                
                aria-label="menu"
              >
                <svg
                  width={31}
                  height={20}
                  viewBox="0 0 31 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <rect width={31} height={2} fill="white" />
                  <rect width={31} height={2} fill="white" />
                  <rect width={31} height={2} fill="white" />
                  <rect y={9} width={31} height={2} fill="white" />
                  <rect y={9} width={31} height={2} fill="white" />
                  <rect y={9} width={31} height={2} fill="white" />
                  <rect y={18} width={31} height={2} fill="white" />
                  <rect y={18} width={31} height={2} fill="white" />
                  <rect y={18} width={31} height={2} fill="white" />
                </svg>
              </button>
            </div>
          </div>
        </div>
      </div>
      <div  />
    </header>
    <div>
      <div >
        <div >
          <template shadowrootmode="open" />
        </div>
        <div >
          <img
            src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/nx_wave_hero.png"
            alt="card_asset"
            
          />
          <div >
            <h1 >
              <span>
                <strong>NX Wave.</strong> The next-gen credit card for those who
                love rewards.
              </span>
            </h1>
            <div >
              <p >
                1% Cashback
                <svg
                  
                  width={12}
                  height={12}
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="m6 0 1.273 4.727L12 6 7.273 7.273 6 12 4.727 7.273 0 6l4.727-1.273L6 0Z"
                    fill="#000"
                    style={{ mixBlendMode: "darken" }}
                  />
                </svg>
                5x Rewards
                <svg
                  
                  width={12}
                  height={12}
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="m6 0 1.273 4.727L12 6 7.273 7.273 6 12 4.727 7.273 0 6l4.727-1.273L6 0Z"
                    fill="#000"
                    style={{ mixBlendMode: "darken" }}
                  />
                </svg>
                Zero Forex Markup
              </p>
            </div>
            <div >
              <div >
                <div >
                  <form>
                    <div >
                      <div >
                        <input
                          
                          placeholder="Enter Phone Number"
                          defaultValue=""
                        />
                        <span  />
                      </div>
                      <button
                        type="submit"
                        
                      >
                        <span>Apply Now</span>
                      </button>
                    </div>
                  </form>
                  <div >
                    <input type="checkbox" defaultChecked="" id="consent-msg" />
                    <label
                      htmlFor="consent-msg"
                      
                    >
                      You agree to be contacted by Uni Cards over Call, SMS,
                      Email or WhatsApp to guide you through your application.
                    </label>
                  </div>
                </div>
              </div>
              <div >
                <div >
                  <a
                    href="https://uni-growth.onelink.me/v6cm/"
                    target="_blank"
                    rel="noopener noreferrer"
                    
                  >
                    <div >
                      <span>Download</span>
                    </div>
                  </a>
                </div>
                <div >
                  <p >
                    Thank you for your interest in the Uni Card.
                    <br /> Download the Uni Cards app now and get your Uni Card
                    in minutes.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <video
          autoPlay=""
          muted=""
          loop=""
          playsInline=""
          
        >
          <source src="/videos/nxt_wave_bg.mp4" type="video/mp4" />
        </video>
      </div>
    </div>
    <div >
      <section>
        <div >
          <div >
            <p
              style={{
                opacity: 0,
                transform: "translateY(100px) translateZ(0)"
              }}
            >
              Earn 1% assured cashback{/* */}{" "}
              <span >on your spends. Get</span> 5X
            </p>
            <p
              style={{
                opacity: 0,
                transform: "translateY(100px) translateZ(0)"
              }}
            >
              more value than cashback{/* */}{" "}
              <span > at the Uni Store. Enjoy</span>
            </p>
            <p
              style={{
                opacity: 0,
                transform: "translateY(100px) translateZ(0)"
              }}
            >
              <span >
                round the clock{" "}
              </span>
              Whatsapp support.{" "}
              <span >And it's</span>
            </p>
            <p
              style={{
                opacity: 0,
                transform: "translateY(100px) translateZ(0)"
              }}
            >
              lifetime free
              <span >
                ; no joining fee, no annual charges.
              </span>
            </p>
          </div>
          <div
            
            style={{
              opacity: 0,
              transform: "translateY(-100px) translateZ(0)"
            }}
          >
            <div
              style={{
                background:
                  "linear-gradient(140.86deg, #65ECD8 11.07%, #FDEF78 80.28%)"
              }}
              
            >
              <img
                alt="down_arrow"
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/down_arrow.svg"
                loading="lazy"
                
              />
            </div>
          </div>
        </div>
      </section>
      <div >
        <div >
          <div
            className="flex md:items-center flex-col-reverse md:flex-row
          
          w-full font-medium justify-center  md:pt-[70px] py-0 md:py-[60px] md:px-8"
          >
            <div >
              <div >
                <div>
                  <span style={{ fontWeight: 700 }}>
                    1% assured cashback on your spends.
                  </span>
                  <span style={{ fontWeight: 400, color: "#9EA7AE" }}>
                    The more you spend, the more you earn.
                  </span>
                </div>
              </div>
              <div >
                <div >
                  Not applicable on fuel purchase, rent &amp; wallet transfers,
                  ATM withdrawals &amp; international transactions.
                </div>
              </div>
            </div>
            <div >
              <div >
                <div >
                  <img
                    src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/one_percent_cashback.png"
                    
                  />
                </div>
              </div>
            </div>
          </div>
          <div
            className="flex md:items-center flex-col-reverse md:flex-row-reverse
          
          w-full font-medium justify-center  md:pt-[70px] py-0 md:py-[60px] md:px-8"
          >
            <div >
              <div >
                <div>
                  <span style={{ fontWeight: 700 }}>
                    5x more value than your cashback,
                  </span>
                  <span style={{ fontWeight: 400, color: "#9EA7AE" }}>
                    {" "}
                    only at the Uni Store.
                  </span>
                </div>
              </div>
            </div>
            <div >
              <div >
                <div >
                  <img
                    src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/five_x_rewards.png"
                    
                  />
                </div>
              </div>
            </div>
          </div>
          <div
            className="flex md:items-center flex-col-reverse md:flex-row
          
          w-full font-medium justify-center  md:pt-[70px] py-0 md:py-[60px] md:px-8"
          >
            <div >
              <div >
                <div>
                  <span style={{ fontWeight: 700 }}>Zero Forex Markup.</span>
                  <br />
                  <span style={{ fontWeight: 400, color: "#9EA7AE" }}>
                    {" "}
                    Go international, without any fees.
                  </span>
                </div>
              </div>
            </div>
            <div >
              <div >
                <div >
                  <img
                    src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/forex_globe.png"
                    
                  />
                </div>
              </div>
            </div>
          </div>
          <div >
            <p>
              Lifetime <span >free. </span>
              <span >
                No joining fee.
              </span>
              <span > No annual charges. </span>
            </p>
          </div>
        </div>
      </div>
    </div>
    <section >
      <img
        src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/circles.png"
        alt="circles"
        
      />
      <div >
        <div >
          <p >
            We’ve all heard of instant groceries, now say hello to
            <span >
              <br /> instant credit.
            </span>
          </p>
          <p >
            0% hassle, 100% paperless. Get your <br /> Uni Card instantly.
          </p>
        </div>
        <div >
          <img
            src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/app_screen_1.webp"
            alt=""
            loading="lazy"
            
          />
        </div>
      </div>
      <div >
        <div >
          <p >
            With Uni,{" "}
            <span >
              <br />
              you’re always in control.
            </span>
          </p>
          <p >
            Set your own payment limits. Choose how and where you spend. Lock
            and unlock your card. All right from the app.{" "}
          </p>
        </div>
        <div >
          <img
            src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/app_screen_2.webp"
            alt=""
            loading="lazy"
            
          />
        </div>
      </div>
    </section>
    <section >
      <div >
        <div >
          <div >
            <div >
              <img
                
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/whatsapp_bubble.webp"
                alt=""
              />
            </div>
          </div>
          <div >
            <p >
              Help, just a WhatsApp away. Anytime, Anyday.
            </p>
            <p >
              <span>
                During hectic work hours. On lazy sunday mornings. In the thick
                of night. Anytime.
              </span>
            </p>
          </div>
        </div>
        <div >
          <div >
            <div >
              <img
                
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/rupee_bubble.webp"
                alt=""
              />
            </div>
          </div>
          <div >
            <p >
              {" "}
              No hidden charges, no last minute surprises.
            </p>
            <p >
              <span>
                100% money back guarantee if a charge is applied without your
                knowledge.
              </span>
            </p>
          </div>
        </div>
        <div >
          <div >
            <div >
              <img
                
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/antivirus_bubble.webp"
                alt=""
              />
            </div>
          </div>
          <div >
            <p >
              {" "}
              Super secure. Because we care about your money.
            </p>
            <p >
              <span>
                <img
                  style={{ marginTop: 24 }}
                  src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/pcidss_cert.svg"
                  alt="PCI Logo"
                />
              </span>
            </p>
          </div>
        </div>
      </div>
    </section>
    <section >
      <div >
        <p >
          <span>
            At Uni, we’re committed to{" "}
            <span >
              {" "}
              delivering an unmatched credit experience{" "}
            </span>{" "}
            for millions of Indians.
          </span>
        </p>
        <p >
          On this mission, we’ve partnered with some of the best in the
          business.
        </p>
        <div >
          <div >
            <img
              
              src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/SBM.svg"
              alt="sbm bank"
            />
          </div>
        </div>
      </div>
    </section>
    <div >
      <p >
        Please note that to stay compliant with RBI guidelines, we have
        discontinued Pay 1/3rd and Pay 1/2 cards for the time being.
      </p>
    </div>
    <section >
      <div >
        <p >
          Download now to get started
        </p>
        <div >
          <a
            href="https://unicards.onelink.me/KRuW/"
            
          >
            <svg
              viewBox="0 0 16 16"
              focusable="false"
              
            >
              <path
                d="M0.813438 0C0.793438 0.062 0.773438 0.123333 0.773438 0.205333V15.6913C0.773438 15.836 0.814104 15.918 0.896104 16L8.5061 7.938L0.813438 0ZM9.01944 8.472L10.9868 10.502L8.1781 12.102C8.1781 12.102 4.6081 14.1313 2.63944 15.2613L9.01944 8.472ZM9.55344 7.918L11.6641 10.1127C11.9948 9.92867 14.5781 8.45133 14.9468 8.246C15.3361 8.02067 15.2968 7.71267 14.9661 7.54867C14.6601 7.37867 12.0768 5.89867 11.6861 5.65867L9.55077 7.92L9.55344 7.918ZM9.0181 7.38467L11.0074 5.272L8.1681 3.65133C8.1681 3.65133 3.53944 1.00333 1.99944 0.119333L9.01944 7.38467H9.0181Z"
                fill="black"
              />
            </svg>
            Google Play
          </a>
          <a
            href="https://uni-growth.onelink.me/v6cm/"
            
          >
            <svg
              viewBox="0 0 16 16"
              focusable="false"
              
            >
              <g clipPath="url(#clip0)">
                <path
                  d="M4.71676 15.6999C4.40143 15.4893 4.12143 15.2313 3.88743 14.9333C3.63209 14.6246 3.39543 14.2999 3.17809 13.9639C2.66743 13.2173 2.26809 12.4006 1.99143 11.5399C1.65809 10.5386 1.49609 9.57994 1.49609 8.64194C1.49609 7.59527 1.72276 6.68194 2.16409 5.91527C2.49076 5.31527 2.97743 4.81327 3.56409 4.4606C4.13076 4.10727 4.79076 3.91393 5.45743 3.9006C5.69076 3.9006 5.94409 3.93393 6.21076 4.0006C6.40409 4.05393 6.63743 4.1406 6.92409 4.24727C7.29076 4.38727 7.49076 4.47393 7.55743 4.49393C7.77076 4.57394 7.95076 4.60727 8.09076 4.60727C8.19743 4.60727 8.35076 4.57394 8.52076 4.5206C8.61743 4.48727 8.80076 4.42727 9.06076 4.31393C9.31809 4.2206 9.52209 4.1406 9.68409 4.0806C9.93076 4.00727 10.1694 3.9406 10.3841 3.90727C10.6441 3.86727 10.9021 3.85393 11.1494 3.87393C11.6228 3.90727 12.0561 4.00727 12.4428 4.15393C13.1228 4.42727 13.6714 4.85394 14.0808 5.4606C13.9074 5.56727 13.7474 5.69127 13.5974 5.82727C13.2728 6.11394 12.9974 6.45394 12.7774 6.8306C12.4908 7.34394 12.3441 7.92394 12.3481 8.5106C12.3581 9.2326 12.5414 9.86727 12.9081 10.4173C13.1661 10.8173 13.5108 11.1599 13.9308 11.4413C14.1374 11.5813 14.3188 11.6779 14.4908 11.7413C14.4108 11.9913 14.3228 12.2346 14.2208 12.4746C13.9894 13.0126 13.7141 13.5279 13.3874 14.0146C13.0994 14.4346 12.8728 14.7479 12.7008 14.9546C12.4328 15.2746 12.1741 15.5146 11.9141 15.6859C11.6274 15.8759 11.2908 15.9766 10.9461 15.9766C10.7128 15.9866 10.4794 15.9566 10.2568 15.8919C10.0634 15.8286 9.87276 15.7573 9.68609 15.6766C9.49076 15.5873 9.28876 15.5113 9.08276 15.4499C8.82943 15.3833 8.56943 15.3513 8.30676 15.3519C8.04009 15.3519 7.78009 15.3853 7.53343 15.4486C7.32676 15.5073 7.12676 15.5793 6.92876 15.6653C6.64876 15.7819 6.46543 15.8586 6.35876 15.8919C6.14276 15.9559 5.92143 15.9946 5.69876 16.0086C5.35209 16.0086 5.02943 15.9086 4.70809 15.7086L4.71676 15.6999ZM9.28609 3.39327C8.83276 3.61993 8.40209 3.71593 7.97076 3.68393C7.90409 3.25327 7.97076 2.8106 8.15076 2.32593C8.31076 1.9126 8.52409 1.53927 8.81743 1.20593C9.12409 0.859268 9.49076 0.572601 9.90409 0.365934C10.3441 0.139268 10.7641 0.0192676 11.1641 -0.000732422C11.2174 0.452601 11.1641 0.899268 10.9974 1.37927C10.8454 1.80593 10.6188 2.19927 10.3308 2.5526C10.0408 2.89927 9.68076 3.18593 9.27343 3.3926L9.28609 3.39327Z"
                  fill="black"
                />
              </g>
              <defs>
                <clipPath id="clip0">
                  <rect width={16} height={16} fill="white" />
                </clipPath>
              </defs>
            </svg>
            App Store
          </a>
        </div>
      </div>
    </section>
    <section>
      <div >
        <div >
          <p>Uni maintains the highest level of security standards</p>
          <img
            src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/pcidss_cert.svg"
            
            width="80px"
            loading="lazy"
            alt=""
          />
        </div>
      </div>
      <footer
        as=""
        
        id="footer"
      >
        <div >
          <div >
            <div >
              <div >
                <svg
                  
                  viewBox="0 0 85 60"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  style={{ display: "block" }}
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M54.894 39.5039H48.6641V19.77H54.894V39.5039ZM30.4021 25.9999V39.504H24.1723V22.3647C24.1723 21.6765 24.4457 21.0165 24.9322 20.5301C25.4188 20.0435 26.0787 19.77 26.7669 19.77H42.3312C43.0195 19.77 43.6793 20.0435 44.1657 20.5301C44.6523 21.0165 44.9257 21.6765 44.9257 22.3647V39.504H38.696V25.9999H30.4021ZM14.2345 33.2742V19.7702H20.4644V36.9095C20.4644 37.5977 20.1909 38.2576 19.7044 38.744C19.2179 39.2307 18.5579 39.5041 17.87 39.5041H2.30544C1.61752 39.5041 0.957379 39.2307 0.470894 38.744C-0.0157201 38.2576 -0.289062 37.5977 -0.289062 36.9095V19.7702H5.9407V33.2742H14.2345Z"
                    fill="white"
                  />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M65.289 19.7708C65.289 21.8685 63.5858 23.5717 61.4882 23.5717C59.3905 23.5717 57.6875 21.8685 57.6875 19.7708C57.6875 17.6732 59.3905 15.9702 61.4882 15.9702C63.5858 15.9702 65.289 17.6732 65.289 19.7708Z"
                    fill="#00DCC8"
                  />
                </svg>
              </div>
              <div >
                <p >
                  Indiqube Sigma No.3/B, Nexus Koramangala 3rd Block SBI Colony,
                  Koramangala, Bengaluru, Karnataka 560034
                </p>
                <p >
                  Contact Us:
                  <a href="tel: 080 68216821"> {/* */}080 68216821</a>
                </p>
                <p >
                  Email:
                  <a href="mailto:%20care@uni.club"> {/* */}care@uni.club</a>
                </p>
              </div>
            </div>
          </div>
        </div>
        <div  />
        <div >
          <p >
            <a
              href="https://www.sbmbank.co.in/aboutus/grievance-redressal-mechanism.php"
              
            >
              <u>Grievance Redressal Mechanism</u> - SBM Bank India
            </a>
          </p>
        </div>
        <div  />
        <div >
          <div >
            <div >
              <a
                
                href="https://www.instagram.com/uni_cards/"
                target="_blank"
                rel="noopener noreferrer"
              >
                Instagram
              </a>
              <img
                width="6px"
                height="6px"
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/footer-arrow.png"
                alt="arrow"
                
              />
            </div>
            <div >
              <a
                
                href="https://www.linkedin.com/company/uni-cards"
                target="_blank"
                rel="noopener noreferrer"
              >
                LinkedIn
              </a>
              <img
                width="6px"
                height="6px"
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/footer-arrow.png"
                alt="arrow"
                
              />
            </div>
            <div >
              <a
                
                href="https://twitter.com/cards_uni"
                target="_blank"
                rel="noopener noreferrer"
              >
                Twitter
              </a>
              <img
                width="6px"
                height="6px"
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/footer-arrow.png"
                alt="arrow"
                
              />
            </div>
            <div >
              <a
                
                href="https://www.facebook.com/Uni-Cards-104179288358904"
                target="_blank"
                rel="noopener noreferrer"
              >
                Facebook
              </a>
              <img
                width="6px"
                height="6px"
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/footer-arrow.png"
                alt="arrow"
                
              />
            </div>
            <div >
              <a
                
                href="https://careers.uni.cards/"
                target="_blank"
                rel="noopener noreferrer"
              >
                Careers
              </a>
              <img
                width="6px"
                height="6px"
                src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/footer-arrow.png"
                alt="arrow"
                
              />
            </div>
          </div>
          <div >
            <span data-testid="cc-kfs">
              <a
                
                href="https://www.uni.cards/credit-card-key-facts"
                target="_blank"
              >
                Credit Card KFS
              </a>
              <span >|</span>
            </span>
            <span data-testid="nxwave-tnc">
              <a
                
                href="https://www.uni.cards/docs/Credit_Card_MITC.pdf"
                target="_blank"
              >
                Credit Card T&amp;Cs
              </a>
              <span >|</span>
            </span>
            <span data-testid="uni-tnc">
              <a
                
                href="https://www.uni.cards/privacy-policy-main"
                target="_blank"
              >
                Uni T&amp;Cs
              </a>
              <span >|</span>
            </span>
            <span data-testid="nbfc-tnc">
              <a
                
                href="https://www.uni.cards/nbfc-terms"
                target="_blank"
              >
                Lending Partner TnCs
              </a>
            </span>
          </div>
        </div>
        <div  />
      </footer>
    </section>
    <div >
      <div >
        <div >
          <button >
            <span>Apply now</span>
            <img
              src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/right_arrow.svg"
              alt="right_arrow"
              width="21px"
              
            />
          </button>
        </div>
      </div>
    </div>
  </div>
  <iframe
    height={0}
    width={0}
    style={{ display: "none", visibility: "hidden" }}
    src="./Uni Cards_ Next-gen Credit Cards & Rewards_files/activityi.html"
  />
  <button id="gpt-text-summarize-button">
    <img src="data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%20128%20128%22%3E%0A%20%20%20%20%3Cg%20fill%3D%22none%22%20fill-rule%3D%22nonzero%22%20transform%3D%22translate(5%205)%22%3E%0A%20%20%20%20%20%20%20%20%3Ccircle%20cx%3D%2259%22%20cy%3D%2259%22%20r%3D%2259%22%20fill%3D%22%2310A37F%22%20stroke%3D%22%2310A37F%22%20stroke-width%3D%229%22%20%2F%3E%0A%20%20%20%20%20%20%20%20%3Cpath%20fill%3D%22%23E5E7EB%22%0A%20%20%20%20%20%20%20%20%20%20%20%20d%3D%22M76.397%2067.004c-1.545%205.815-5.642%208.127-10.356%209.47-.464.148-.386.595-.386.595l.695%204.623s.077.373.696.298C83.66%2080.2%2094.714%2068.047%2092.78%2053.285c-2.01-10.215-10.356-14.167-17.93-13.123-7.652%201.193-12.908%208.053-11.67%2015.434%201.004%206.561%206.646%2011.184%2013.215%2011.408Z%22%20%2F%3E%0A%20%20%20%20%20%20%20%20%3Cpath%20fill%3D%22%23FFF%22%0A%20%20%20%20%20%20%20%20%20%20%20%20d%3D%22M26.16%2055.596c1.005%206.487%206.722%2011.11%2013.211%2011.408-1.622%205.815-5.562%208.127-10.352%209.47-.464.148-.387.595-.387.595l.773%204.623s.077.373.695.298c16.534-1.79%2027.736-13.943%2025.65-28.705-1.93-10.215-10.198-14.167-17.846-13.123C30.254%2041.355%2025%2048.215%2026.16%2055.596Z%22%20%2F%3E%0A%20%20%20%20%3C%2Fg%3E%0A%3C%2Fsvg%3E" />
  </button>
</>

  );
}

export default App;
